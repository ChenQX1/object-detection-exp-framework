...
class Anchors(nn.Module):
    ...
    def forward(self, image):
        ...
        for idx, p in enumerate(self.pyramid_levels):
            anchors         = generate_anchors(base_size=self.sizes[idx], ratios=self.ratios, scales=self.scales) #generate single anchor
        ...
        if self.use_cuda:
            ...
            all_anchors = all_anchors.cuda(device)
        return all_anchors
...
def generate_anchors(base_size=16, ratios=None, scales=None):
    ...
    num_anchors = len(ratios) * len(scales) # total anchor pre anchor

    # initialize output anchors , type:(x,y,w,h)
